      ...
      {options.map((opt) => (
        <button key={opt} onClick={() => onChange(opt)}
          className={cn("text-xs px-3 py-1.5 rounded-full border transition-colors",
            value === opt ? "bg-secondary text-white border-secondary dark:bg-primary dark:border-primary" : "border-border dark:border-dark-border text-text-secondary hover:border-secondary/40")}>
          {opt}
        </button>
      ))}
    </div>
